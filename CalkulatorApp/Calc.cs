...
namespace CalkulatorApp
{
    class Calc
    {
        ...
        private static void PopFunction(Stack<double> Operands, Stack<char> Functions)
        {
            ...
            else
            {
                ...
                switch (Functions.Pop())
                {
                    ...
                    case '/': Operands.Push(A / B);
                        break;
                    case '^': Operands.Push(Math.Pow(A, B));
                        break;
                    case '%': Operands.Push(A % B);
                        break;
                }
        ...
        private static int GetPriority(char op)
        {
            switch (op)
            {
                ...
                case '-':
                    return 3;
                default:
                    throw new Exception("Niprawidłowy znak");
            }
        }

        // nadaje liczby typu double lub operacji
        ...
        // zczytuje liczby typu double => 2.456436 zapisuje jako 1 laczba
        private static string ReadDouble(string s, ref int pos)
        {
            string res = "";
            //pracuje dopoky podany symbol jest liczbą
